...
class SyncAgentPermissionsTests(unittest.TestCase):
    ...
    def test_codex_legacy_managed_block_is_migrated(self):
        module = load_module()
        original = "\n".join(
            [
                'prefix_rule(pattern=["local"], decision="allow")',
                module.LEGACY_CODEX_BEGIN,
                'prefix_rule(pattern=["old"], decision="allow")',
                module.LEGACY_CODEX_END,
                "",
            ]
        )
        updated, changed = module.merge_codex_rules(original, [["git"]])
        ...
        self.assertTrue(changed)
        self.assertIn(module.CODEX_BEGIN, updated)
        self.assertIn(module.CODEX_END, updated)
        self.assertNotIn(module.LEGACY_CODEX_BEGIN, updated)
        self.assertIn('prefix_rule(pattern=["local"], decision="allow")', updated)
        self.assertIn('prefix_rule(pattern=["git"], decision="allow")', updated)
