...
CODEX_BEGIN = "# BEGIN managed by agent-common permissions"
CODEX_END = "# END managed by agent-common permissions"
LEGACY_CODEX_BEGIN = "# BEGIN managed by agent-permissions-sync"
LEGACY_CODEX_END = "# END managed by agent-permissions-sync"


def default_allowlist_path():
    return Path.home() / ".config" / "agent-common" / "permissions" / "common-allowlist.json"
...
def merge_codex_rules(existing_text, prefix_rules):
    ...
    elif LEGACY_CODEX_BEGIN in existing_text and LEGACY_CODEX_END in existing_text:
        before, rest = existing_text.split(LEGACY_CODEX_BEGIN, 1)
        _, after = rest.split(LEGACY_CODEX_END, 1)
        updated = before + block + after
    else:
